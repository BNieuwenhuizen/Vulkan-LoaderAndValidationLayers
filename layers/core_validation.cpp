...
static bool ValidateAccessMaskPipelineStage(VkAccessFlags access_mask, VkPipelineStageFlags stage_mask) {
    ...
    if (stage_mask & VK_PIPELINE_STAGE_ALL_COMMANDS_BIT) return true;
    if (0 == access_mask) return false;
    ...
    while (access_mask) {
        index = __builtin_ctz(access_mask);
        assert(index >= 0);
        supported = AccessMaskToPipeStage[index] & stage_mask;
